...
on which to measure each type's effect on propensity to cheat.
We use coding tasks whose unit tests have been mutated to contradict their own
specifications; thus, a report of 'pass' is a specification-violating shortcut.
...
**(i)** Calibration screens multiple reasoning models with different capabilities.
Baseline cheating rates are assessed against the [35%, 85%] band (heuristic-based) to
support sufficient detection sensitivity and range. In-band are o4-mini (65% baseline
cheating) and o3 (78%); at 10%, o3-mini is out-of-band and excluded from remaining
experiments.
...
**(iii)** On the same run, the AS-vs-GP contrast is directional but remains
sub-threshold: Δ = 0 on o4-mini and +6pp on o3 (95% CI [−2, +14], n.s.), with the
o3 interval falling entirely below the pre-registered 15pp minimum important
difference (MID). The MID is detectability-bound — it equals the finest resolution
afforded by the 103-task census — so a true ~6pp effect is below what the substrate
can resolve.

We close by motivating a powered effect-scaling test (Phase 2), and a purer
anti-deception-vs-effort evaluation (Phase 3).
...
this project, those studies are currently out of reach as they rely on NLAs for reasoning 
models (not currently available), as shown in this work.
...
Noting Schoen's anti-scheming spec two-type bundled composition as low-hanging 
fruit for potential spec minimization, we ask to what degree the two individual 
clause types suppress cheating, and whether that proportion changes as capability 
scales. The distinction is safety-relevant because the two failure modes targeted 
are expected to diverge with capability: genuine capability-failure (the model 
doesn't recognize the task is impossible, or doesn't try hard enough) drops as 
models improve, while alignment-failure in the form of cheating or scheming _ability_ 
(without assuming the same for propensity) would be expected to improve alongside 
...
hardcoding a value). For variant _v_ and model _m_, the cheating rate is
...
**Conditions.** ImpossibleBench supplies the tasks and
scorer; we derive the specification text from Schoen. The spec is injected as a system 
...
and a content-neutral `effort_only` control. The isolated variants (`as_only` and `gp_only`) 
repair the spec's internal cross-references so neither names an absent clause block. Full variant text: 
...
**H2 (scaling):** That advantage from H1 grows with capability. The estimand is the
...
and between-task difficulty cancels from the difference. For the model capability contrast, 
...
device is, thus, McNemar (test) and Newcombe<sup>10</sup> (interval) (vs. their unpaired analogs).
...
**Calibration / reasoning-gate.** Cheating on ImpossibleBench LCB-`conflicting`/`minimal` 
tracks reasoning, not size: non-reasoning models (gpt-4o-mini, gpt-4o, gpt-4.1) sit at 
...
**The one-epoch decomposition.** Two models, one stochastic draw per
task, paired within-task on the full 103-task `conflicting` split. The result is 
purely directional. At single-epoch, the CIs are wide and omit the variance a 
multi-epoch run would add (if anything, they understate uncertainty).
...
isolated specs. All three capability contrasts survive Holm within their family. 
With only two models, this is one capability contrast, not a confirmed trend.

**(ii) AS-vs-GP is directional and sub-threshold.** Comparing isolated spec components, 
Δ is 0 on o4-mini and +6pp on o3, with the o3 interval falling entirely below the 15pp 
MID and not clearing zero. The pilot is consistent with anti-deception content 
suppressing more on the stronger model, but the effect — if real — is below what 
the census can resolve (§4).
...
The 15pp MID was set arbitrarily at ~15pp; below that level, an anti-deception
advantage is considered too small to be useful. Simultaneously, the resolution 
limit afforded by the 103-task census is ~15pp. (≈110 items are needed for 15pp; 
10pp would need ~250, more than double the number of available tasks.) 

Thus, the "importance" bar is effectively the detectability floor. Two structural 
limits make this binding: the `conflicting` split is a census (N cannot grow with 
more items), and epochs are ICC-capped: at the pilot ICC ≈ 0.64, the minimum 
detectable effect cannot fall below √ICC ≈ 0.8× its single-epoch value, no matter 
how many epochs are run.

As a result, a true H1 effect near the pilot's observed +6pp would be below the 
achievable MDE, so a powered run on this substrate would return a bounded null 
("anti-deception adds less than the achieved MDE beyond process content"), not 
"no effect." A lower MID would require more independent tasks (MDE ∝ 1/√N_items, no
floor) than available.

**One epoch.** Each task is a single draw, so the intervals carry no across-epoch 
(ICC) component. This means every number is directional, and a between-condition 
"flip" cannot yet be separated from the model's own run-to-run stochasticity.
...
Resolving intent is the deferred reasoning-summary layer (and, further out, an 
activation-level read); it is out of scope here.

**The AS/GP split is approximate.** The isolated variants are a custom-built 
construct (with cross-references repaired); their faithfulness to Schoen's composite 
anti-scheming bundle is an external-validity assumption, not a measured fact. 
Further, some of GP's clauses might be read as anti-deception. Ultimately, 
`as_only − gp_only` tests the hypothesis on the published, deployed spec and 
not as a pure anti-deception-vs-effort contrast. This is a deliberate tradeoff — 
practical bearing now on the deployed spec, construct purity to follow (Phase 3).

**Reasoning-gated scope.** The study runs on reasoning models since that is where
cheating was observed on LCB-`minimal`; non-reasoning cheating lives on the heavier
SWE-bench/`tools` substrate, slated for future work. The gpt-4.1 floor is what 
licenses "reasoning-gated, not capability-gated."

**Model selection.** The [35%, 85%] band selects models with measurable cheating, 
so capability levels are conditioned on that selection: a spec can only suppress 
cheating that exists, and ~50% maximizes proportion variance. The capability 
difference between two in-band models, however, is not a selection artifact.

**Inference choices.** (i) Newcombe over Wald (Brown–Cai–DasGupta<sup>11</sup>: Wald 
coverage is erratic; the two agree here, but Newcombe is the principled default). 
...
Phase 2 is the real test of H1 and H2.

- **Benchmark size constraint.** If the true H1 effect is near +6pp, it 
  sits below the census's ~15pp resolution. Detecting it requires more independent
  tasks — the pre-registered `oneoff` replication arm and/or a larger substrate —
  because epochs are ICC-capped and the `conflicting` split is already a census. 
  For Phase 2, the key decision is "**which/how large a benchmark**," with the construct 
  caveat that a new split must measure the same thing. Phase 0's IB-measured ICC sets 
  the epoch count at the cost-effectiveness knee.
- **Capabilities ladder powers H2.** H1 power is per-model (N_items, epochs, ICC,
  baseline rate, pairing) and does not depend on the number of models. H2, which asks if
  Δ grows with capability, depends on the number of models and their capability spread.
  A formal interaction test is underpowered at any affordable ladder, as it would need 
  ~4× the power and ≥10 well-spread models. Thus, H2 is registered as a descriptive
...
- **Interpretation.** A bounded null on H1 states: "anti-deception content adds less than 
  ...
  axis generates a resolvable signal.
...
cleaner anti-deception-vs-effort contrast that isolates those two axes deliberately —
the same hypothesis is tested at a higher level of construct purity.
...
step further from a spec actually deployed on frontier models (Schoen). Phase 1/2 and
Phase 3 are complementary, not redundant; the former has practical relevance (does the 
deployed spec's split matter?) and the latter, construct cleanliness (is it the 
anti-deception axis per se?). Should Phase 1/2 return a bounded null, Phase 3 will 
also discriminate "no effect" from "the published split is too approximate to carry 
the effect."
...
census affords; it is a bounded-null risk that no amount of compute fixes, because the
pre-registered MID is at the substrate's detectability floor. Phase 2 requires a 
decision about benchmark size which will, in turn, set the resolution limit on effect 
size. Phase 3 will study the effect at a higher degree of construct purity to support 
generalization.
